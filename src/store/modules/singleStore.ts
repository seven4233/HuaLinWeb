...
import { SingleItem, SingleOption, SubmitQuestionParams } from "@/api/sys/model/questionModel";
import { getSingleListAPI, submitQuestionAPI } from "@/api/sys/question";
...
export const useSingleStore = defineStore(
  ...
  () => {
    ...
    const selectedAction = (selectedOption: SingleOption) => {
      ...
      // 找到目标题目
      let targetQuestion: any = singleList.value?.find(
    ...
    // 答对题数
    const correctCount = computed(() => singleList.value?.filter(item => item.answer === item.your).length)
    // 答对题目列表
    const correctList = computed(() => singleList.value?.filter(item => item.answer === item.your))
