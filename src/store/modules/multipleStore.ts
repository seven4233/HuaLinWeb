...
import { MultipleItem, MultipleOption,  SubmitQuestionParams } from '@/api/sys/model/questionModel';
import { getMulListAPI, submitQuestionAPI } from '@/api/sys/question';
...
export const useMultipleStore = defineStore(
  ...
  () => {
    ...
    const getMultipleListAction = async (bankId: number) => {
      //   获取多选题
      const res = await getMulListAPI(bankId);
      console.log(res); 
    ...
    const addFinishedQuestion = async (params: SubmitQuestionParams) => {
      ...
      // 重新获取 进度
      const r = await getMulListAPI(params.bankId);
